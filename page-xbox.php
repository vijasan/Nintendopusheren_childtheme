    ...
    <section id="primary" class="content-area spil entry">
        ...
        <script>
            ...
            async function getJson() {
                // Fetch all custom post types with the slug "xbox"
                const url = "https://vijasan.dk/kea/10_eksamen/nintendopusheren/wp-json/wp/v2/xbox?per_page=100";
            ...
            function createButtons() {
                categories.forEach(cat => {
                    //console.log(cat.id);
                    if (cat.name == "Xbox" || cat.name == "Xbox 360" || cat.name == "Xbox One" || cat.name == "Xbox Series X") {
                        document.querySelector("#filtrering").innerHTML += `<button class="filter" data-xbox="${cat.id}">${cat.name}</button>`
                    }
                })
            ...
            function displayGames() {
                ...
                xbox.forEach(xbox => {
                    // Check filterspil
                    if (filterspil != "alle" && !xbox.categories.includes(parseInt(filterspil)))
                        return;
                    if (filterselectedemballage != "alt" && xbox.emballage != filterselectedemballage)
                        return;
                    const clone = skabelon.cloneNode(true).content;
                    clone.querySelector(".spil-title").textContent = xbox.title.rendered;
                    clone.querySelector(".pris").textContent = xbox.pris;
                    clone.querySelector(".konsol").textContent = xbox.konsol;
                    clone.querySelector(".emballage").textContent = xbox.emballage + " emballage";
                    clone.querySelector(".picture").src = xbox.billede;
                    clone.querySelector(".picture").alt = xbox.billednavn;
                    clone.querySelector(".spil-container").addEventListener("click", () => visDetaljer(xbox));
                    liste.appendChild(clone);
                });
            }

            function visDetaljer(xbox) {
                popup.style.display = "flex";
                popup.querySelector(".spil-title").textContent = xbox.title.rendered;
                popup.querySelector(".pris").textContent = xbox.pris;
                popup.querySelector(".konsol").textContent = xbox.konsol;
                popup.querySelector(".emballage").textContent = xbox.emballage + " emballage";
                popup.querySelector(".picture").src = xbox.billede;
                popup.querySelector(".picture").alt = xbox.billednavn;
            }
            ...
            function filter() {
                filterspil = this.dataset.xbox
                if (this.dataset.xbox === undefined) {
                    filterspil = "alle"
                }
